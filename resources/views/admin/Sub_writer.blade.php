                ...
                <table class="table">
                  ...
                  <tbody>
                   @foreach ($data as $writer)
                      ...
                      <tr>
                        <td><img style="width:150px;" src=" {{url('img/'.$writer->image_path)}} "></td>
                        <td> {{$writer->name}} </td>
                        <td>{{$writer->email}}</td>
                        <td>{{$writer->created_at	}}</td>
                        <td>{{$writer->updated_at	}}</td>
                        <td>Writer</td>
                        @if(Cache::has('writer-is-online-' .$writer->id))
                        <td><span class="text-success">Now</span></td>
                       <td>

                        <span class="text-success">Online</span>
                    @else 
                    <td> {{ Carbon\Carbon::parse($writer->last_seen)->diffForHumans() }}</td>
                        <td><span class="text-secondary">Offline</span></td>
                    @endif
                       </td>
                        <td class="text-right py-0 align-middle">
                          <div class="btn-group btn-group-sm">
                            <a href="{{url('admin/edit_Sub_writer/'.$writer->id)}} " onclick="return confirm('Are you sure?')" class="btn btn-info"><i class="fas fa-edit"></i></a>
                            <a href=" {{url('admin/delete_writer/'.$writer->id)}}" onclick="return confirm('Are you sure?')" class="btn btn-danger"><i class="fas fa-trash"></i></a>
                          </div>
                        </td>
                      </tr>
                      @endforeach
